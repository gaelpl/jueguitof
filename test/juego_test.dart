...
void main() {
  ...
  group('Función de Extracción de Celdas por Región', () {
    ...
    test('extrae solo los números asignados ignorando casillas vacías', () {
      final resultado = extraerValoresDeBloque(tableroPrueba, Region.rojoNoroeste);
      expect(resultado, equals([3, 5]));
    });
 
    test('extrae todos los números de la zona amarilla aunque estén separados', () {
      final resultado = extraerValoresDeBloque(tableroPrueba, Region.amarillo);
      expect(resultado, equals([1, 6]));
    });
 
    test('retorna lista vacía si el bloque solo contiene casillas sin rellenar', () {
      final resultado = extraerValoresDeBloque(tableroPrueba, Region.verdeEste);
    ...
    test('permite componer la extracción con una regla de validación', () {
      final valoresRojo = extraerValoresDeBloque(tableroPrueba, Region.rojoNoroeste);
